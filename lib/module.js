...
module.exports = function sparingCenter(moduleOptions) {
  ...
  plugins.forEach(plugin => {
    this.addPlugin({
      src: path.resolve(sparingPluginsDir, plugin + '.ts'),
  ...
  if (options.axiosI18nHeader) {
    const headerName =
      typeof options.axiosI18nHeader === 'boolean'
        ? 'Accept-Language'
   ...
   */
  const CSS_DIR =
    '~/node_modules/@sparing-software/nuxt-sparing-center/lib/css/'
  this.options.styleResources = defaultValue(this.options.styleResources, {
    scss: []
  })

  if (options.styleResources) {
    this.options.styleResources.scss.push(
      '@/assets/sass/_vars.scss',
      '@/assets/sass/_mixins.scss'
    )

    if (options.sassUtilsCollection) {
      this.options.styleResources.scss.unshift(
        CSS_DIR + 'sass-utils-collection.scss'
      )
    }
